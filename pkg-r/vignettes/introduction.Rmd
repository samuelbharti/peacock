---
title: "Getting Started with peacock"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting Started with peacock}
...
peacock is a simple package that helps you start new projects quickly, in R, Quarto or Python. Instead of manually creating folders and files every time, peacock sets them up for you with sensible defaults.
...
- `init_shiny()` - Create a Shiny app structure
- `init_analysis()` - Scaffold a reproducible analysis project
- `init_quarto()` - Scaffold a Quarto project
- `init_python()` - Scaffold a Python project
- `init_template()` - Pull a GitHub repository in as a template
- `tool_review_template()` - Set up a tool comparison project
- `init_changelog_md()` - Add a changelog file
- `peacock_templates()` - List the built-in templates
...
When you run `init_shiny()`, you get:

**Main files:**
- `ui.R` - User interface definition with navbar structure
- `server.R` - Server logic (empty but ready to use)
- `global.R` - Load libraries, data, and shared objects

**Organized folders:**
- `modules/` - Modular Shiny components (includes example module)
- `userInterface/` - UI components split by page
...
- `.gitignore` - Pre-configured for R projects
...
If you have complete project templates on GitHub, pull them down with `init_template()`.
...
This downloads the template, extracts it, and sets it up in your chosen directory. The function automatically cleans up temporary files.

### Available templates

`peacock_templates()` lists the built-in registry, which includes `"shiny"` for a
full Shiny application and `"cgds"` for the UAB CGDS research layout. Any
`"owner/repo"` works too, optionally pinned with `@ref`, so the registry is a
shortcut rather than a limit.

## Tracking Changes

Keep a log of what you've done with `init_changelog_md()`.

```{r changelog, eval = FALSE}
...
All functions have a `confirm` parameter. Set it to `FALSE` when using peacock in automated scripts:

```{r tips_confirm, eval = FALSE}
...
Yes. The files created are normal files on your computer. Change them however you want. If you create a template you like, consider making it a GitHub repository and using `init_template()` to reuse it.
...
1. Try creating a test project with `init_shiny()`
...
For more examples and updates, visit: https://www.samuelbharti.com/peacock/r/
