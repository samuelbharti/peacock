...
trunk_x = rx + 40
rows += (
    f'<line x1="{trunk_x}" y1="{row_y0 - 16}" x2="{trunk_x}" '
...
for i, (name, is_dir, hl) in enumerate(tree):
    ...
    # connector tick
    rows += (
        f'<line x1="{trunk_x}" y1="{y}" x2="{trunk_x + 16}" y2="{y}" '
        f'stroke="{P["border"]}" stroke-width="2"/>'
    )
    gx = trunk_x + 24
    if is_dir:
        rows += (
            f'<rect x="{gx}" y="{y - 8}" width="16" height="12" rx="2.5" '
            f'fill="{P["folder"]}"/>'
        ...
        rows += (
            f'<path d="M {gx + 2},{y - 10} H {gx + 9} L {gx + 14},{y - 5} V {y + 5} '
    ...
    rows += (
        f'<text x="{tx}" y="{y + 5}" font-family="{MONO}" font-size="19" '
        f'fill="{col}" font-weight="{weight}">{esc(name)}</text>'
    )
    if hl:
        pill_x = tx + 128
        rows += (
            f'<rect x="{pill_x}" y="{y - 13}" width="34" height="21" rx="10.5" '
            f'fill="{P["ai_bg"]}"/>'
