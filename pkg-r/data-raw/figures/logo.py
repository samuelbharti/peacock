...
Usage: python logo.py <variant> <out.svg> [simple]
       variant in {indigo,maroon,cream}; `simple` drops fine detail for
       small favicon sizes (16/32 px)."""
...
def hex_pts(r):
    return [
        (cx + r * math.cos(rad(a)), cy - r * math.sin(rad(a)))
...
def build(variant, simple=False):
    V = VARIANTS[variant]
    line = V["line"]
    C = {**FILL, **V}
    ...
    def leaf(rb, rt, w=20):
        mid = (rb + rt) / 2
        return (
            f"M {rb:.1f},0 Q {mid:.1f},{-w:.1f} {rt:.1f},0 "
    ...
    def eyespot():
        ...
        if not simple:
            ...
            for i in range(n):
                ...
                dots += (
                    f'<circle cx="{dx:.1f}" cy="{dy:.1f}" r="1.7" fill="{C["gold"]}"/>'
                )
            shine = (
        ...
        return (
            dots + f'<ellipse rx="26" ry="19.5" fill="{C["eye_gold"]}" '
            ...
            f'stroke="{line}" stroke-width="1"/>' + shine
        )
    ...
    body_path = (
        "M 261,300 C 251,301 246,320 246,342 C 246,372 221,398 222,430 "
        "C 223,470 240,512 261,536 C 282,512 299,470 300,430 "
        "C 301,398 276,372 276,342 C 276,320 271,301 261,300 Z"
    )
    # scalloped breast pattern (rows of small arcs) + neck bands
    scallop = ""
    if not simple:
        for row, (yy, half, step) in enumerate(
            [(392, 34, 17), (420, 30, 15), (448, 24, 12)]
        ):
            xs = [261 + s for s in range(-half, half + 1, step)]
            for x in xs:
                scallop += (
                    f'<path d="M {x - step * 0.5:.1f},{yy} Q {x:.1f},{yy + 9} '
                    f'{x + step * 0.5:.1f},{yy}" fill="none" stroke="{C["gold"]}" '
                    f'stroke-width="1.3" opacity="0.9"/>'
                )
        for yy in (322, 336):
            scallop += (
                f'<path d="M 250,{yy} Q 261,{yy + 5} 272,{yy}" fill="none" '
    ...
    for dx, ty in [(-20, 226), (0, 216), (20, 226)]:
        crest += (
            f'<line x1="261" y1="266" x2="{261 + dx * 0.5:.0f}" y2="{ty + 8}" '
    ...
    if not simple:
        crest += (
            f'<path d="M 261,272 q 5,6 0,13 q -5,-7 0,-13 Z" fill="{C["vermilion"]}" '
...
simple = len(sys.argv) > 3 and sys.argv[3] == "simple"
with open(out, "w", encoding="utf-8") as f:
    f.write(build(variant, simple))
print("wrote", out)
