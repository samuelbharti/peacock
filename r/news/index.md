...
- `init_package()` scaffolds a full R package with DESCRIPTION,
  NAMESPACE, starter function, testthat tests, R-CMD-check workflow,
  air.toml, pkgdown config, and agent files.

### Other

- Restructured to monorepo layout: R package lives in `pkg-r/`, Python
  CLI in `pkg-py/`, shared template registry in `shared/`.

- Added `peacock-init` Python CLI companion
